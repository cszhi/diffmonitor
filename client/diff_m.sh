...
LOG="$DIR/log"
LOCK_FILE="$DIR/diff_m.lock"
API_TOKEN=${API_TOKEN:-changeme}
CURL="curl -s --connect-timeout 10 --max-time 60 -H X-API-Token:$API_TOKEN"
...
exec 9>"$LOCK_FILE"
flock -n 9 || { echo "diffmonitor is already running"; exit 0; }
...
grep -v "^#" "$DIR/list.conf" |while read name type action mode;
do
	sleep 1
